...
**V1.1 定稿**（当时的最新版本），AC-01 ~ AC-20 与 V-1 ~ V-9 均已通过。
...
**V1.2 定稿**（当时的最新版本），AC-01 ~ AC-20、V-1 ~ V-9（V1.1）与 V-1 ~ V-6（V1.2）均已通过。
...
**V1.4 定稿**（当时的最新版本），AC-01 ~ AC-20、V1.1/V1.2/V1.3/V1.4 各自的验收清单均已通过。
...
动态背景已实现并经真机验收通过，计划与历次返工记录见 `doc/V1.5改进计划.md`。

| 项 | 内容 |
|---|---|
| 内容 | 在原有竖向渐变之上，**4 个半透明光斑缓缓上浮并左右摆动**，透明度另有一层「呼吸」。**用色为当前主题色的 `primary` + `tertiary`**，4 套主题色各自呈现自己的色相（浅蓝+紫 / 黄绿+绿 / 玫红+橙 / 琥珀+橄榄） |
| 冻结取值 | 峰值透明度 **0.40**（唯一调节旋钮，上限 0.5）、上浮周期 20 s（四路 17~29 s）、半径 0.22~0.34 屏宽、呼吸峰值 60%~100%、光斑数量 4 |
| 架构要点 | 这是本项目**首次引入常驻动画**：进度由**自己数帧**推进（`withFrameNanos` 累计帧间隔，不经 Compose 的动画时长折算），前台门控**只决定时间是否推进**（不决定画什么），非前台时进度冻结、回前台接着走 |
| 版本 | `versionCode = 11`，`versionName = "1.5"`（**计划交付写的是 6**；实际经 5 次返工逐版递增为 7 / 8 / 9 / 10 / 11，其中 9 是临时诊断版，未作为交付物） |
| 提交标签 | `v1.5.0` |
| 单测 | 203 个 JVM 单测全绿（V1.4 为 194；新增 9 条动态背景断言） |
| 真机验收 | V-1 ~ V-8 全部通过（含 V-7「提示高亮与背景同色相时描边仍一眼可见」）。判定依据是开发者在真机上的观察 |
| SRS | 同步至 **V1.10**：`FR-13.9` 由 V1.8 新增，V1.9 把「低透明度」改为「半透明」，V1.10 把「中性色」改为「主题色」；`NFR-1.6`（背景不得干扰棋盘辨识） |
| 交付物 | `dist/LianLianKan-v1.5.0-release.apk` 9.13 MB，签名密钥与前五版相同（`SHA-1 d7d97ec7…`，可直接覆盖升级） |

**V1.5 是当前的最新版本**，AC-01 ~ AC-20、V1.1/V1.2/V1.3/V1.4/V1.5 各自的验收清单均已通过。

**这个版本的价值有一半在过程记录里**：真机反馈连续四轮都是反例 —— 三次「看不到背景」，
排查结论前两次都不对（先是把「像素差算对」当成「看得见」，后是把库里一个**真实存在但并非
本次原因**的机制当成根因），最后靠**给应用装探针**才定位到「绘制、位置、时钟全都正常，
只是浓度太低」；第四次是「灰色光斑太难看」，即**「可见」与「好看」是两件事**。
两条教训写在 `doc/V1.5改进计划.md` §17.4 与 §18.4：**像素差是「可见」的必要条件而非充分条件；
「氛围型」视觉参数（浓度、颜色）的验收判据只能是真机 A/B，不能是本机计算值。**
本机无模拟器，唯一能做的是把「已经否掉的做法」写成单测断言防止回退。

**后续注意**：改动动态背景时，`DefaultBackgroundSpec.maxAlpha` 是**唯一**的浓度旋钮；
`AnimatedBackgroundTest` 里有三条来自返工的断言（可见性下限、时钟循环、用色必须为主题色且不退化
成灰），改坏会直接变红。新增界面时留意：任何一层画了不透明底色就会把背景整层盖住
（`Scaffold` 的默认 `containerColor` 正是如此，V1.5 曾因此在两屏上完全看不到背景）。

#### V1.6 定稿（2026-09-14）

**关于界面**已实现并经真机验收通过，计划见 `doc/V1.6改进计划.md`。

| 项 | 内容 |
|---|---|
| 内容 | 新增关于界面：应用图标 + APP 名称 + 「版本 1.6（构建 12）」+ 一句话简介 + 版权行（「© 2026 leid」） |
| 入口 | **设置界面底部**（「清除进度」下方，方案 Q1 = A）—— 主菜单按钮数不变，既有观感不受影响；入口位置唯一 |
| 版本号来源 | 打开 `buildFeatures { buildConfig = true }`，读 `BuildConfig.VERSION_NAME` / `VERSION_CODE`。**版本号的唯一真源仍是 `app/build.gradle.kts`**，这是本版本唯一有技术含量的决策 |
| 新增防线 | `AboutInfoTest` 3 条，其中一条专门防「把版本号写死在代码里」：断言展示文字必须包含 `BuildConfig` 的版本名与构建号 —— 这个界面是「用来告诉别人这是哪一版」的，它一旦说谎比没有它更糟 |
| 约束遵守 | **未新增任何依赖与权限**（为做出带箭头的设置项行而引入 Material 图标依赖被判为不划算）；关于界面显式传 `containerColor = Color.Transparent`，**不遮盖动态背景**（V1.5 §15.1 的事故教训） |
| 版本 | `versionCode = 12`，`versionName = "1.6"` |
| 提交标签 | `v1.6.0` |
| 单测 | 206 个 JVM 单测全绿（V1.5 为 203） |
| 真机验收 | V-1 ~ V-7 全部通过（含「4 主题 × 3 皮肤下动态背景在关于界面仍可见」） |
| SRS | 同步至 **V1.11**：5.1 补「关于界面」与设置界面的「关于（入口）」；新增 **FR-15 关于**（5 条，含「不联网、不申请新权限」的 P0 约束） |
| 交付物 | `dist/LianLianKan-v1.6.0-release.apk`（9.13 MB），签名密钥与前六版相同（`SHA-1 d7d97ec7…`，可直接覆盖升级） |

**V1.6 是当前的最新版本**，AC-01 ~ AC-20、V1.1/V1.2/V1.3/V1.4/V1.5/V1.6 各自的验收清单均已通过。

**后续注意（本版本定稿时新发现的维护点）**：版本号目前出现在三处 —— `app/build.gradle.kts`
（唯一真源）、README 的版本表、`dist/` 的文件名。后两者靠人工同步，单测只能守住代码侧，
因此**发版清单里应包含「核对 README 版本表与 dist 文件名」**。
另：图标只显示自适应图标的前景层，看着会偏小（画布 108dp、内容占中间 66dp）；
若日后要更好看，换成「背景层 + 前景层按 1.636 倍放大 + 圆角裁剪」的组合图标即可（约六行）。
...
| FR-15 | 关于 | V1.6 | 单测 A-1 ~ A-3、手动 V-1 ~ V-7 |
