...
**SRS 解读项（需确认）**：SRS UC-03 步骤 3.b.ii 对「图案相同但不可连通」只说「视为错误操作（**同 3.a 的反馈**）」。3.a 的反馈包含「将新点击的牌设为选中」，因此实现按**字面读法**让 `NoPath` 与 `WrongType` 行为一致：都把第二张牌设为选中并让连击归零，区别只在提示文案（「无法连通」vs「图案不同」）。

另一种常见的连连看手感是「保留第一张选中，方便玩家换一个搭档」。两者都说得通，当前按 SRS 字面实现并有用例固定。**若你偏好后者，改动只在 `GameSession.select` 的 `NoPath` 分支两行。**
