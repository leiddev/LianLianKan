...
- [ ] M0-1 在 `libs.versions.toml` 补齐第 2.2 节所列依赖（版本取最新稳定版）
- [ ] M0-2 启用 Compose。**注意 AGP 9 采用内置 Kotlin 支持**（不再显式应用 `org.jetbrains.kotlin.android`），Compose 编译器的启用方式需在本任务中实测确认；若 `android.buildFeatures.compose = true` 单独不够，则补充 Compose 编译器插件（参考 [AGP 9 内置 Kotlin 迁移说明](https://github.com/android/skills/blob/392fd3bc/build/agp/agp-9-upgrade/references/android/build/migrate-to-built-in-kotlin.md)、[Compose 编译器迁移指南](https://kotlinlang.org/docs/compose-compiler-migration-guide.html)）
- [ ] M0-3 `MainActivity` 改为 `ComponentActivity` + `setContent`，渲染一句占位文本
- [ ] M0-4 移除 `appcompat` / `material`(Views) 依赖，`themes.xml` 与 `values-night/themes.xml` 改用 Material 3 主题（已确认，见 D-2）
- [ ] M0-5 在 `AndroidManifest.xml` 的 `<application>` 内声明 `MainActivity`，并锁定竖屏（FR-1.4）
- [ ] M0-6 建立第 5 节的包结构骨架（空目录 + `package` 声明）
- [ ] M0-7 跑通 `./gradlew :app:assembleDebug` 与 `./gradlew :app:testDebugUnitTest`
...
- [ ] M1-1 模型：`Position(row, col)`、`Tile(id, type, row, col, state)`、`Board(rows, cols, tiles)`、`GamePhase`、`Settings`、`Progress`（对应 SRS 7.1）
- [ ] M1-2 `LevelConfig(level, cols, rows, typeCount, timeLimit, hintCount, shuffleCount)`
- [ ] M1-3 `LevelCatalog`：按 SRS 附录 9.3 录入 10 关配置
- [ ] M1-4 **图案张数分配器**：按已确认决策 D-1（见 9.1 节）实现——`base = (tileCount / typeCount)` 向下取到最近偶数，前 `k` 类各分得 `base + 2` 张，其余各类分得 `base` 张，`k = (tileCount - base × typeCount) / 2`。已用脚本对 10 关全部验证：`k` 均为非负整数，每类张数为偶数，总和等于牌总数
- [ ] M1-5 `BoardGenerator`：构造成对图案集合 → 洗牌 → 填充（FR-3.2），保证同一关多次生成布局不同（FR-3.3）
- [ ] M1-6 单测：牌总数为偶数、每类张数为偶数、类型分布符合预期、生成结果随机性（AC-01）
...
- [ ] M2-1 棋盘扩展为 `(rows+2) × (cols+2)`，外圈视为永久空白，统一处理绕行（SRS 9.1）
- [ ] M2-2 `PathFinder.find(a, b): PathResult?`，`PathResult` 含 `points: List<Position>` 与 `turns: Int`
- [ ] M2-3 0 拐点：同行/同列且中间全空（FR-5.2）
- [ ] M2-4 1 拐点：候选拐点 `(a.row, b.col)`、`(b.row, a.col)` 双向可达性检查（FR-5.3）
- [ ] M2-5 2 拐点：沿 a 的行/列扫描可直达空白点，再以 0/1 拐点条件桥接到 b（FR-5.4）
- [ ] M2-6 非法输入（越界、同一张牌、已消除）安全返回 `null`，不抛异常（NFR-2.4）
- [ ] M2-7 单测矩阵：
...
- [ ] M3-1 `DeadlockDetector.hasMove(board): Boolean` —— 遍历剩余牌，按类型分组，组内两两试连通（FR-6.1）
- [ ] M3-2 `ShuffleService.shuffle(board)` —— 收集剩余牌的类型集合 → 重排 → 校验可解（FR-3.4）
- [ ] M3-3 有限次重试（建议 50 次）后仍不可解，回退到「成对重排」策略。**注意需避免无限循环**
- [ ] M3-4 洗牌只重排**未消除**的位置，牌数与类型分布保持不变
- [ ] M3-5 单测：人为构造死局棋盘 → 断言检测为死局；洗牌后断言存在可连通对；连续洗牌 200 次断言恒可解（AC-08）
...
- [ ] M4-1 `ScoreEngine`：单次得分 = `10 × 倍率`，倍率 = `min(1 + 0.1 × (combo - 1), 2.0)`（FR-8.1、SRS 9.2）
- [ ] M4-2 连击窗口 3 秒，超时归零；错误选中、洗牌道具均归零；提示道具不归零（FR-8.2/8.3、FR-9.4）
- [ ] M4-3 通关时间奖励 = 剩余秒数 × 2（FR-8.4）
- [ ] M4-4 `GameTimer`：**注入时间源**（`() -> Long` 或 `Clock`）而非直接读系统时间，便于单测与暂停恢复（SRS 2.6、5.2）
- [ ] M4-5 剩余时间 < 30s 的视觉提示状态位（FR-7.4，P2，逻辑层先留 flag）
- [ ] M4-6 单测：倍率上限 2.0 的边界、连续 20 次消除的累计得分、暂停期间计时不推进、超时归零
...
- [ ] M5-1 `GameSession`（纯 Kotlin 聚合根），对外仅暴露 `select(position): SelectResult` 与 `pause/resume/restart/useHint/useShuffle`
- [ ] M5-2 消除事务顺序：校验选中 → 判连通 → 移除牌 → 计分/连击 → 死局检测 → 必要时自动洗牌 → 判定通关（FR-4、UC-03 主流程）
- [ ] M5-3 错误反馈语义与 SRS 一致：图案不同 → `WrongType`（并把第二张设为选中）；图案相同但不通 → `NoPath`（FR-4.4/4.5）
- [ ] M5-4 道具：提示找一对可连通牌并返回坐标，不消耗连击；洗牌消耗次数、清空选中、连击归零；用尽时返回不可用（FR-9.1 – 9.5）
- [ ] M5-5 自动洗牌不消耗玩家道具次数，并置「已自动重排」提示位（FR-6.2）
- [ ] M5-6 状态机 `READY / PLAYING / PAUSED / WIN / LOSE` 与 SRS 3.4 完全一致；重开/重试不重置解锁进度（FR-10.7）
- [ ] M5-7 `GameViewModel`：持有 `StateFlow<GameUiState>`，用 `viewModelScope` 驱动计时；接收 UI 事件
- [ ] M5-8 单测：会话级集成测试——模拟完整一局（自动寻解直到清盘）断言 `WIN`；模拟超时断言 `LOSE`；模拟死局断言自动洗牌

---

### M6 棋盘 UI 与动画（2 – 3 人日）

- [ ] M6-1 棋盘容器：按 `cols × rows` 等分可用宽高，单元格边长取 `min(可用宽 / cols, 可用高 / rows)`
- [ ] M6-2 `TileView`：圆角卡片 + 阴影 + 按压缩放反馈（FR-13.3）；选中态边框光晕/背景高亮（FR-13.4）
- [ ] M6-3 牌面图案：emoji / 矢量图标，保证小尺寸下可辨识（FR-13.7）
- [ ] M6-4 `PathOverlay`：`Canvas` 绘制连线，支持圆角折线与渐变（FR-13.5）；路径取 M2 输出的点序列
- [ ] M6-5 连线动画：沿线快速绘制（约 150 ms）后启动消除动画
- [ ] M6-6 消除动画：缩放 + 淡出（FR-13.6），动画期间该牌不可再点（FR-4.7）
- [ ] M6-7 错误反馈：两牌抖动 / 红闪
- [ ] M6-8 顶部 HUD：倒计时、得分、连击（< 30s 变色）；底部操作条：提示、洗牌、暂停（用尽置灰，FR-9.3）
- [ ] M6-9 无障碍最小点击尺寸检查：8 列布局在 360dp 宽屏上单格约 42–45dp，低于 48dp 建议值（SRS 未要求无障碍，记为已知取舍，见第 9 节 P-3）
...
- [ ] M7-1 导航图：`menu → levelSelect → game`，配 `Navigation Compose`
- [ ] M7-2 主菜单（FR-1.1 – 1.3）：标题、开始/继续（有进度显示「继续」）、关卡选择、设置、退出
- [ ] M7-3 关卡选择（FR-2.1 – 2.4）：10 关卡片，锁定态、最佳分、解锁联动
- [ ] M7-4 暂停弹窗（FR-10.1）：继续 / 重开本关 / 返回主菜单
- [ ] M7-5 结算面板（FR-10.2 – 10.6）：得分、最高连击、时间奖励、是否刷新最佳分；通关显示「下一关」（末关显示「全部通关」），失败显示「重试本关」「返回主菜单」
- [ ] M7-6 设置页（FR-11.1 – 11.5）：音效、振动、主题模式、皮肤、清除进度（二次确认）
- [ ] M7-7 全局反馈：Snackbar 承载「道具用尽」「无法连通」「已自动重排」等提示（SRS 5.1）

---

### M8 本地持久化（1 人日）

- [ ] M8-1 `DataStore` 实例与键定义，完整覆盖 SRS 7.2 数据字典 9 个键
- [ ] M8-2 `SettingsRepository`（音效/振动/主题/皮肤）、`ProgressRepository`（解锁关卡、各关最佳分、总分、进行中关卡、schema 版本）
- [ ] M8-3 写入时机：关卡结束、进度解锁、设置变更（DR-2）
- [ ] M8-4 主题模式驱动 Compose `isSystemInDarkTheme()` 分支（配合 M10）
- [ ] M8-5 「继续游戏」读取进行中关卡（FR-1.2、FR-14.3，P1）
- [ ] M8-6 进行中关卡的快照粒度：按 DR-3 先做「仅保存关卡编号」，若成本可控再升级为完整快照
- [ ] M8-7 单测：仓库以 `DataStore` 的测试替身（临时文件）验证读写与默认值
...
SRS FR-3.1 要求「每类图案数量均为偶数」，但 9.3 表中以下 6 个关卡的牌总数无法被图案种类整除：
