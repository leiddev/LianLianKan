...
**关于界面**已实施（`versionCode = 12`、`versionName = "1.6"`，206 个 JVM 单测全绿），
计划见 `doc/V1.6改进计划.md`，实施结果见其 §14。

| 项 | 内容 |
|---|---|
| 新增界面 | `ui/about/AboutScreen.kt`：应用图标 + APP 名称 + 「版本 1.6（构建 12）」+ 一句话简介 + 版权行（「© 2026 leid」，Q3 由开发者给定） |
| 入口 | **设置界面底部**（「清除进度」下方，方案 Q1 = A）—— 主菜单按钮数不变，既有验收观感不受影响 |
| 版本号来源 | 打开 `buildFeatures { buildConfig = true }`，读 `BuildConfig.VERSION_NAME` / `VERSION_CODE`。版本号的**唯一真源**仍是 `app/build.gradle.kts`；这是本版本唯一有技术含量的决策 |
| 新增断言 | 3 条（`AboutInfoTest`），其中一条专门防「把版本号写死在代码里」：断言展示文字必须包含 `BuildConfig.VERSION_NAME` / `VERSION_CODE` |
| 约束遵守 | 不新增任何依赖与权限（项目无 Material 图标依赖，入口沿用既有 `TextButton` 样式）；关于界面显式传 `containerColor = Color.Transparent`，**不遮盖动态背景**（V1.5 §15.1 的事故教训） |
| SRS | 同步至 **V1.11**：5.1 界面清单补「关于界面」与设置界面的「关于（入口）」；新增 **FR-15 关于**（5 条，含「不联网、不申请新权限」的 P0 约束） |
| 仍需真机确认 | V-1 ~ V-7（见 `doc/V1.6改进计划.md` §5.2）：入口与返回、名称与桌面图标一致、版本号与安装包一致、版权完整、**4 主题 × 3 皮肤下背景仍可见**、大字号不截断、回归 |

**真机验收通过后再在本节补「定稿」记录并打标签** —— 沿用 V1.2 / V1.4 / V1.5 的做法：
**先验收、后定稿、再打标签**。
