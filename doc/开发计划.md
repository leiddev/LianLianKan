...
动态背景已实施（`versionCode = 6`、`versionName = "1.5"`，200 个 JVM 单测全绿），
实施结果与验收清单见 `doc/V1.5改进计划.md` §14。**真机验收通过后再在本节补「定稿」记录并打标签** ——
沿用 V1.2 / V1.4 的做法：**先验收、后定稿、再打标签**（V1.3 曾因跳过这一步而漏掉标签，见其 §13）。
