...
**SRS 观察项（新增，非阻塞）**：SRS 9.3 表的**牌总数并非单调递增** —— 第 3 关 8×8=64，第 4 关 6×10=**60**，序列为 48,48,64,60,64,80,80,80,96,96，在 3→4 处回落一次；同时第 1~4 关限时均为 180s，并非逐关递减。整体难度趋势（9.4 第 2 条「棋盘变大 + 图案种类变多 + 限时略减」）仍然成立：图案种类 8→13 单调不减，限时 180→150 单调不增。已按 SRS 原表实现，并用 `LevelCatalogTest.记录 SRS 9-3 表中第 4 关牌总数低于第 3 关这一非单调点` 显式断言，避免日后被误判为回归。**是否需要调整第 4 关棋盘规格，待确认。**
