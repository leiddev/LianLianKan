...
**仍待实现**：皮肤携带配色（SRS FR-11.4 的 Level 2）—— 需先把 `ColorContrastTest` 参数化为
「每套皮肤 × 深浅色」，属独立一轮工作。
...
| AC-15 | 主题 | **自动** + 真机 | 自动：`ColorContrastTest`（WCAG 对比度断言）。真机：浅色/深色/跟随系统三态切换正常，深色下牌面与文字清晰可辨 |
| AC-16 | 离线 | 真机 | 开飞行模式，全流程可玩（应用无网络权限，理论上不可能联网） |
| AC-17 | 算法单测 | **自动** | 182 个 JVM 单测全绿 |
