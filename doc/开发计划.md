...
动态背景已实施（**修订 1**，`versionCode = 7`、`versionName = "1.5"`，202 个 JVM 单测全绿），
实施结果与验收清单见 `doc/V1.5改进计划.md` §14（修订 0）与 **§15（修订 1，当前有效）**。
首次实现的可见性未被真机认可（「完全看不出背景在动」），修订 1 记录了排查出的两个独立原因：
关卡选择与设置两屏的 `Scaffold` 默认不透明底色把背景整层盖住，以及光斑的峰值像素差只有 1~2 级
（约束立在了「透明度」这个配方数字上，而真正的判据是合成后的像素差）。
