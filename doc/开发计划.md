...
动态背景已实施（**修订 4**，`versionCode = 11`、`versionName = "1.5"`，203 个 JVM 单测全绿），
实施结果与验收清单见 `doc/V1.5改进计划.md` §14（修订 0）、§15~§17（三次「看不到」的返工）与
**§18（修订 4：改用主题色）**。

**这个版本的真机反馈连续四轮都是有价值的反例**：三次「看不到背景」的排查结论都不对
（前两次分别是「像素差算得对但感知阈值判断错」与「误把库里的一个真实机制当成本次原因」，
最后靠**给应用装探针**才定位到「绘制、位置、时钟全都正常，只是浓度太低」）；第四次是
「灰色光斑太难看」，即**「可见」与「好看」是两件事**。最终取峰值透明度 **0.40**
（唯一旋钮）与主题色 `primary` + `tertiary`。

**教训（§17.4 / §18.4）**：像素差是「可见」的必要条件而非充分条件；「氛围型」视觉参数
（浓度、颜色）的验收判据只能是真机 A/B，不能是本机计算值 —— 本机无模拟器，
唯一能做的是把「已经否掉的做法」写成单测断言防止回退。
