...
动态背景已实施（**修订 2**，`versionCode = 8`、`versionName = "1.5"`，203 个 JVM 单测全绿），
实施结果与验收清单见 `doc/V1.5改进计划.md` §14（修订 0）、§15（修订 1）与 **§16（修订 2，当前有效）**。
两轮真机反馈都是「看不到背景」，原因分别是：光斑的峰值像素差只有 1~2 级（把约束立在了
「透明度」这个配方数字上，而真正的判据是合成后的像素差）；以及动画根本没有推进 ——
Compose 会按系统的「动画程序时长缩放」折算动画时长，缩放为 0 时进度被直接取到动画结尾，
光斑全部停在屏幕之外。**这个设置是全局的，若设备上它为 0，本项目其它 Compose 动画
（牌位移动画、消除动画、错误抖动、提示条）也会一直是瞬变** —— 请先确认该设置，
再重过与动画有关的既往验收项（§16.4）。
