...
| [`doc/V1.3改进计划.md`](doc/V1.3改进计划.md) | 主题改为 4 种主题色（配色经标准 M3 色调板算法生成），深色模式整体移除。**已定稿，V-1 ~ V-4 / V-6 / V-7 真机验收通过（tag `v1.3.0`，回溯打标签）** |
| [`doc/V1.4改进计划.md`](doc/V1.4改进计划.md) | APP 图标（矢量 + 光栅双轨）与启动白屏。**已定稿，V-1 ~ V-8 真机验收通过** |
| [`doc/V1.5改进计划.md`](doc/V1.5改进计划.md) | 动态背景（含性能、省电与干扰控制）。**已定稿，V-1 ~ V-8 真机验收通过**（历经 4 次返工，§15~§19 是完整的过程记录） |
| [`doc/V1.6改进计划.md`](doc/V1.6改进计划.md) | 关于界面（APP 名称 / 版本号 / 版权信息）。**已定稿，V-1 ~ V-7 真机验收通过** |
...
| **V1.5** | 已定稿（tag `v1.5.0`） | **动态背景**：在原有竖向渐变之上，**4 个半透明光斑缓缓上浮并左右摆动**，透明度另有一层「呼吸」（上浮周期 17~29 s、峰值透明度 0.40、**用色为当前主题色的 `primary` + `tertiary`**，4 套主题色各自呈现自己的色相）。这是本项目**首次引入常驻动画**，也是踩坑最深的一个版本：真机连续四轮反馈 ——「看不到」×3（前两次的排查结论都不对，最后靠**给应用装探针**才定位到「绘制、位置、时钟都正常，只是浓度太低」），以及颜色被否（「灰色光斑太难看」，改为主题色）。全过程、历代取值对照表与教训见 [`doc/V1.5改进计划.md`](doc/V1.5改进计划.md) §15~§19。`versionCode = 11`，203 个 JVM 单测全绿 |
| **V1.6** | 已定稿（tag `v1.6.0`） | **关于界面**：显示应用图标、APP 名称、「版本 1.6（构建 12）」、一句话简介与版权信息；入口在**设置界面底部**。版本号取自 `BuildConfig`（即 `build.gradle.kts`，打开 `buildConfig` 后才可用），并有单测专门防「把版本号写死在代码里」。本版本不新增任何依赖与权限。`versionCode = 12`，206 个 JVM 单测全绿 |
...
| **背景浓度与棋盘边界的取舍**（V1.5） | 光斑的浓度在本项目里是**唯一**的调节旋钮（`DefaultBackgroundSpec.maxAlpha`，现 0.40）。它同时决定两件事：看得见（太低 → 真机上注意不到：峰值明暗差约 52 级时被判定「看不到」，107 级以上才可见）与不干扰（太高 → 抢注意力）。**中期取值反而最糟**：背景被压暗到与未选中的牌同亮度时（约 0.15 处）牌的边界会发糊，而这个亮度带恰好是径向渐变必经之处、无法消除，只能让峰值远离它。现取 0.40 后，光斑中心把背景压得**比牌更暗**，牌反倒成了「浅色方块浮在彩色薄雾上」，边界比没有背景时更清楚。嫌太浓 → 0.28~0.32；嫌太淡 → 0.46。 |
| **背景光斑与「选中 / 提示 / 错误」共用色相**（V1.5 修订 4） | 光斑改用主题色（`primary` + `tertiary`）后，背景与承载游戏语义的强调色**必然共用色相** —— 这推翻了早期「背景只用中性色」的策略。复核后认为可以接受：背景是 0.40 的半透明薄雾而非实色，而选中/提示/错误的**描边**是 tone 40 实色，与薄雾的对比度仍有约 4.6:1；**牌本身不透明**，薄雾盖不到它，选中牌与未选中牌的差别不受影响。**`error` 系仍然禁止**（红色在连连看里就是「错误」语义），这条由单测 `背景用色必须取自主题色 - 不得退化成灰` 钉住（同时禁止「退回灰色」）。 |
...
206 个 JVM 单测，覆盖：连通判定的 0/1/2 拐点与外侧绕行、棋盘生成、洗牌可解性、死局检测、计分与连击、倒计时、游戏会话状态机、偏好映射、反馈开关与波形、**主题色对比度与色相**、**皮肤与主题色注册表不变量**、**设置加载信号与启动画面超时兜底**、**关于界面的版本信息**（排版与退化输入；**并断言展示文字必须包含 `BuildConfig` 的版本名与构建号 —— 防「把版本号写死在代码里」**）、**动态背景的约束不变量**（光斑数量 3~8、峰值透明度 ≤ 0.5、上浮周期 ≥ 15 s、半径区间合法、呼吸不归零，以及三条来自真机返工的断言：**「每个光斑在其峰值处都看得见」**—— 断言的是合成后的**相对亮度差**（下界取自真机实测的两个锚点之间：约 52 级看不见、约 137 级看得清）；**「进度按上浮周期循环、初始相位互相错开」**—— 断言的是时钟本身；**「背景用色必须取自主题色、不得退化成灰」**—— 断言的是颜色必须是主题色且合成后仍带色相，把「已经被否掉的灰色方案」钉成回退红线）。
