...
| **V1.5** | 已实施（待真机验收） | **动态背景**：在原有竖向渐变之上叠加 **5 个极低透明度的柔和光斑**，各自沿椭圆轨迹缓慢漂移（周期 30 s、透明度上限 0.055、用色仅限中性角色）。这是本项目**首次引入常驻动画**，因此带上了生命周期门控（仅前台运行）与「稳态下每帧零对象分配」的绘制实现。`versionCode = 6`，200 个 JVM 单测全绿 |
...
| **不响应系统的「移除动画」偏好**（V1.5） | 应用的动态背景（缓慢漂移的柔和光斑）**始终运动**：即使玩家在系统设置里开启了「移除动画」（无障碍设置），或在部分省电模式下 `ANIMATOR_DURATION_SCALE` 被置 0，背景仍会动。这是 V1.5 计划 §13 的 **Q3 = B** 决策 —— **有意识接受的取舍**，也是本版本唯一一处已知的无障碍退让。理由是低透明度（≤ 0.08）、超长周期（≥ 20 s）的背景运动对绝大多数用户无干扰，而读取系统偏好要引入一处额外的分支与测试。若日后收到相关反馈，恢复成本约十行：`AnimatedBackground.kt` 里已留有现成的静止分支（非前台时就走它），只需再与一个「是否允许动效」的布尔值相与即可。 |
...
200 个 JVM 单测，覆盖：连通判定的 0/1/2 拐点与外侧绕行、棋盘生成、洗牌可解性、死局检测、计分与连击、倒计时、游戏会话状态机、偏好映射、反馈开关与波形、**主题色对比度与色相**、**皮肤与主题色注册表不变量**、**设置加载信号与启动画面超时兜底**、**动态背景的约束不变量**（光斑数量 ≤ 8、单个光斑透明度 ≤ 0.08、漂移周期 ≥ 20 s、半径区间合法、背景用色不含承载游戏语义的强调色）。
