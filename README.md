...
| [`doc/V1.3改进计划.md`](doc/V1.3改进计划.md) | 主题改为 5 种主题色；皮肤携带配色（FR-11.4 Level 2）。**计划中，有 3 个待确认项** |
...
| 皮肤不含配色 | SRS FR-11.4 的原文是「图案/配色主题」。当前只实现图案集（Level 1）；配色随皮肤变化（Level 2）需要把 `ColorContrastTest` 参数化为「每套皮肤 × 深浅色」，属独立一轮工作。 |
