...
| **V1.5** | 已实施（修订 2，待真机验收） | **动态背景**：在原有竖向渐变之上，**4 个半透明光斑缓缓上浮并左右摆动**，透明度另有一层「呼吸」（上浮周期 17~29 s、峰值透明度 0.13、用色仅限中性角色）。这是本项目**首次引入常驻动画**，也因此踩了两个坑：先后两轮真机反馈都是「看不到背景」——第一轮是光斑的像素差只有 1~2 级（约束立在了「透明度」这个配方数字上），第二轮是**动画压根没推进**（Compose 会按系统的「动画程序时长缩放」折算动画时长，缩放为 0 时进度被直接取到动画结尾，光斑全部停在屏幕之外）；两次排查见 [`doc/V1.5改进计划.md`](doc/V1.5改进计划.md) §15、§16。`versionCode = 8`，203 个 JVM 单测全绿 |
...
| **不响应系统的「移除动画」偏好**（V1.5） | 应用的动态背景（缓缓上浮的半透明光斑）**始终运动**：即使玩家在系统设置里开启了「移除动画」（无障碍设置），或在部分省电模式下 `ANIMATOR_DURATION_SCALE` 被置 0，背景仍会动。这是 V1.5 计划 §13 的 **Q3 = B** 决策 —— **有意识接受的取舍**，也是本版本唯一一处已知的无障碍退让。理由是低透明度（峰值 ≤ 0.13）、长周期（≥ 17 s）的背景运动对绝大多数用户无干扰，而读取系统偏好要引入一处额外的分支与测试。**实现层是刻意做到这一点的**：背景的进度由自己数帧推进（`withFrameNanos` 累计帧间隔），不走 Compose 的动画时长折算，因此系统把动画时长缩放设为 0 也照样运动（V1.5 修订 2 的根因正是这个设置，详见 `doc/V1.5改进计划.md` §16）。若日后要反过来「尊重」该偏好，只需把 `AnimatedBackground` 里那个 `advancing` 布尔值再与一个「是否允许动效」的条件相与即可（那一层门控现在只决定时间是否推进，不决定画什么）。 |
| **系统「动画程序时长缩放」为 0 时，本 APP 的其它动画也会变成瞬变**（V1.5 排查发现） | `MotionDurationScale` 是全局的：只要系统把「动画程序时长缩放」设为 0（开发者选项，或无障碍里的「移除动画」），Compose 里所有基于动画规格的过渡都会**瞬间完成** —— 包括牌位移动画（洗牌滑动）、消除动画（缩放 + 淡出）、错误抖动、按压回弹、提示条进出。这不是本项目的 bug，而是应用与系统设置的交互。动态背景已改为不受该设置影响；其余动画仍遵循它（这是符合预期的行为）。**排查动画类问题前请先确认这个设置的值。** |
...
203 个 JVM 单测，覆盖：连通判定的 0/1/2 拐点与外侧绕行、棋盘生成、洗牌可解性、死局检测、计分与连击、倒计时、游戏会话状态机、偏好映射、反馈开关与波形、**主题色对比度与色相**、**皮肤与主题色注册表不变量**、**设置加载信号与启动画面超时兜底**、**动态背景的约束不变量**（光斑数量 3~8、峰值透明度 ≤ 0.15、上浮周期 ≥ 15 s、半径区间合法、呼吸不归零、背景用色不含承载游戏语义的强调色，以及两条来自真机返工的断言：**「每个光斑在其峰值处都看得见」**—— 断言的是合成后的像素差而非配方数字；**「进度按上浮周期循环、初始相位互相错开」**—— 断言的是时钟本身。前一条对应「光斑在数学上不可见」，后一条对应「动画被系统的动画时长缩放掐死」）。
