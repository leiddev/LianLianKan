...
| **V1.5** | 已实施（修订 4，待真机验收） | **动态背景**：在原有竖向渐变之上，**4 个半透明光斑缓缓上浮并左右摆动**，透明度另有一层「呼吸」（上浮周期 17~29 s、峰值透明度 0.40、**用色为当前主题色的 `primary` + `tertiary`**，4 套主题色各自呈现自己的色相）。这是本项目**首次引入常驻动画**，也是踩坑最深的一个版本：真机连续四轮反馈 ——「看不到」×3（前两次的排查结论都不对，最后靠**给应用装探针**才定位到「绘制、位置、时钟都正常，只是浓度太低」），以及颜色被否（「灰色光斑太难看」，改为主题色）。全过程、历代取值对照表与教训见 [`doc/V1.5改进计划.md`](doc/V1.5改进计划.md) §15~§18。`versionCode = 11`，203 个 JVM 单测全绿 |
