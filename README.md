...
| **V1.5** | 已实施（修订 1，待真机验收） | **动态背景**：在原有竖向渐变之上，**4 个半透明光斑缓缓上浮并左右摆动**，透明度另有一层「呼吸」（上浮周期 17~29 s、峰值透明度 0.10、用色仅限中性角色）。这是本项目**首次引入常驻动画**，因此带上了生命周期门控（仅前台运行）。首次实现的可见性未被真机认可，复盘见 [`doc/V1.5改进计划.md`](doc/V1.5改进计划.md) §15。`versionCode = 7`，202 个 JVM 单测全绿 |
...
| **不响应系统的「移除动画」偏好**（V1.5） | 应用的动态背景（缓缓上浮的半透明光斑）**始终运动**：即使玩家在系统设置里开启了「移除动画」（无障碍设置），或在部分省电模式下 `ANIMATOR_DURATION_SCALE` 被置 0，背景仍会动。这是 V1.5 计划 §13 的 **Q3 = B** 决策 —— **有意识接受的取舍**，也是本版本唯一一处已知的无障碍退让。理由是低透明度（峰值 ≤ 0.10）、长周期（≥ 17 s）的背景运动对绝大多数用户无干扰，而读取系统偏好要引入一处额外的分支与测试。若日后收到相关反馈，恢复成本约十行：`AnimatedBackground.kt` 里已留有现成的静止分支（非前台时就走它），只需再与一个「是否允许动效」的布尔值相与即可。 |
...
202 个 JVM 单测，覆盖：连通判定的 0/1/2 拐点与外侧绕行、棋盘生成、洗牌可解性、死局检测、计分与连击、倒计时、游戏会话状态机、偏好映射、反馈开关与波形、**主题色对比度与色相**、**皮肤与主题色注册表不变量**、**设置加载信号与启动画面超时兜底**、**动态背景的约束不变量**（光斑数量 3~8、峰值透明度 ≤ 0.12、上浮周期 ≥ 15 s、半径区间合法、呼吸不归零、背景用色不含承载游戏语义的强调色，以及**「每个光斑在其峰值处都看得见」**—— 这条断言的是合成后的像素差而非配方数字，V1.5 首次实现正是因为只测了配方而在真机上「看不出在动」）。
