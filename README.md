...
| [`doc/V1.6改进计划.md`](doc/V1.6改进计划.md) | 关于界面（APP 名称 / 版本号 / 版权信息）。**已实施，待真机验收** |
...
| **V1.6** | 已实施（待真机验收） | **关于界面**：显示应用图标、APP 名称、「版本 1.6（构建 12）」、一句话简介与版权信息；入口在**设置界面底部**。版本号取自 `BuildConfig`（即 `build.gradle.kts`，打开 `buildConfig` 后才可用），并有单测专门防「把版本号写死在代码里」。本版本不新增任何依赖与权限。`versionCode = 12`，206 个 JVM 单测全绿 |
